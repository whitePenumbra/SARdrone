...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.btn_Launch.clicked.connect(MainWindow.display_stream)
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.droneMap.load(QUrl.fromLocalFile(source.map_dir + "pointer.html"))
    ...
    def display_stream(self):
        ...
        print("instructions confirmed received.")
    ...
    def get_next_frame(self):
        if self.ready:
            self.ready = False
            self.cvImage = common.next_proc_frame()
            height, width, channel = self.cvImage.shape
            bytes_per_line = 3 * width
            q_image = QImage(self.cvImage.data, width, height, bytes_per_line, QImage.Format_BGR888)
            q_pixmap = QPixmap.fromImage(q_image)

            self.drone_stream.setPixmap(q_pixmap)
            self.ready = True
